...
let parse_function p attribs function_type =
  let pos = p.pos in
  ...
  (* name :: (args...) ... *)
  (*         ^~~~~~~~~     *)
  and _arguments p name =
    let p, arguments = parse_function_arguments p in
    match arguments with
    | None -> p, None
    | Some x ->
        match (peek p 0).kind with
        | Token.Minus -> _type p name x
        | _ -> _body p name x None

  (* name :: (args...) -> type ... *)
  (*                   ^~ ^~~~     *)
  and _type p name arguments =
    let tok1, tok2 = peek p 0, peek p 1 in
    match tok1.kind, tok2.kind with
    | Token.Minus, Token.RArrow when not tok1.has_whitespace_after ->
        ...
        (match type_ with
        | None -> p, None
        | Some _ -> _body p name arguments type_)
  ...
  (* name :: (args...) ... { ... } *) (* name :: (args...) ... $ expr; *)
  (*                       ^~~~~~~ *) (*                       ^ ^~~~^ *)
  and _body p name arguments type_opt =
    let p, body =
      let tok = peek p 0 in
      match tok.kind with
    ...
    | None -> p, None
    | Some x ->
        let function_ast : Ast.t = {
          kind = Ast.Func { name = name
