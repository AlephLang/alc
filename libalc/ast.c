...
static Alc_Vector(Alc_String) to_string(const Alc_Ast *ast)
{
  ...
  case ALC_AST_KIND_ROOT: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = safe_reserve(Alc_Vector(Alc_String), ast->ROOT.toplevel_statements_num);
    array_to_strings(children_vs_v, ast->ROOT.toplevel_statements,
                     ast->ROOT.toplevel_statements_num);
  ...
  case ALC_AST_KIND_EXPR: {
    ...
    alc_vector_push(children_vs_v, to_string(ast->EXPR.lhs));
    alc_vector_push(children_vs_v, to_string(ast->EXPR.rhs));
    alc_vector_push(children_vs_v, to_string(ast->EXPR.operator));
  ...
  case ALC_AST_KIND_PREFIX_EXPR: {
    ...
    alc_vector_push(children_vs_v, to_string(ast->PREFIX_EXPR.operand));
    alc_vector_push(children_vs_v, to_string(ast->PREFIX_EXPR.operator));
  ...
  case ALC_AST_KIND_MODULE: {
    ...
    alc_string_append_cstr(&header, ast->MODULE.name);
    alc_string_append_cstr(&header, "\" }");
    add_to_strings_opt(children_vs_v, ast->MODULE.submodule);
  ...
  case ALC_AST_KIND_IMPORT: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->IMPORT.module));
  ...
  case ALC_AST_KIND_TYPEDEF: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 3);
    add_to_strings_opt(children_vs_v, ast->TYPEDEF.generic_placeholder_type_list);
    alc_vector_push(children_vs_v, to_string(ast->TYPEDEF.aliased_type));
    add_to_strings_opt(children_vs_v, ast->TYPEDEF.attribute_list);
    Alc_String header = alc_string_create_from("TYPEDEF { name: \"");
    alc_string_append_cstr(&header, ast->TYPEDEF.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_EXTERN_FUNC: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 2);
    alc_vector_push(children_vs_v, to_string(ast->EXTERN_FUNC.argument_list));
    add_to_strings_opt(children_vs_v, ast->EXTERN_FUNC.return_type);
    Alc_String header = alc_string_create_from("EXTERN_FUNC { name: \"");
    alc_string_append_cstr(&header, ast->EXTERN_FUNC.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_QUALIFIER: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->QUALIFIER.qualified));
    Alc_String header = alc_string_create_from("QUALIFIER { name: \"");
    alc_string_append_cstr(&header, ast->QUALIFIER.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_STRUCT: {
    ...
    add_to_strings_opt(children_vs_v, ast->STRUCT.attribute_list);
  ...
  case ALC_AST_KIND_UNION: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), ast->UNION.children_num + 1);
    add_to_strings_opt(children_vs_v, ast->UNION.attribute_list);
    array_to_strings(children_vs_v, ast->UNION.children, ast->UNION.children_num);
    Alc_String header = alc_string_create_from("UNION { name: \"");
    alc_string_append_cstr(&header, ast->UNION.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_ENUM: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), ast->ENUM.elements_num + 1);
    add_to_strings_opt(children_vs_v, ast->ENUM.attribute_list);
    array_to_strings(children_vs_v, ast->ENUM.elements, ast->ENUM.elements_num);
    Alc_String header = alc_string_create_from("ENUM { name: \"");
    alc_string_append_cstr(&header, ast->ENUM.name);
    alc_string_append_cstr(&header, "\" is_enum_flags: ");
    alc_string_append_cstr(&header, ast->ENUM.is_enum_flags ? "true }" : "false }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_ENUM_ELEMENT: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    add_to_strings_opt(children_vs_v, ast->ENUM_ELEMENT.expression);
    Alc_String header = alc_string_create_from("ENUM_ELEMENT { name: \"");
    alc_string_append_cstr(&header, ast->ENUM_ELEMENT.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_FUNC: {
    ...
    add_to_strings_opt(children_vs_v, ast->FUNC.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->FUNC.argument_list));
    add_to_strings_opt(children_vs_v, ast->FUNC.return_type);
    alc_vector_push(children_vs_v, to_string(ast->FUNC.body));
    Alc_String header = alc_string_create_from("FUNC { name: \"");
    alc_string_append_cstr(&header, ast->FUNC.name);
  ...
  case ALC_AST_KIND_SCOPE: {
    ...
    alc_string_append_cstr(&header, ast->SCOPE.type);
    alc_string_append_cstr(&header, "\" }");
  ...
  case ALC_AST_KIND_CASE_CHAIN: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), ast->CASE_CHAIN.cases_num + 1);
    array_to_strings(children_vs_v, ast->CASE_CHAIN.cases, ast->CASE_CHAIN.cases_num);
    alc_vector_push(children_vs_v, to_string(ast->CASE_CHAIN.body));
  ...
  case ALC_AST_KIND_CASE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->CASE.expression));
  ...
  case ALC_AST_KIND_ATTRIBUTE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = safe_reserve(Alc_Vector(Alc_String), ast->ATTRIBUTE.arguments_num);
    array_to_strings(children_vs_v, ast->ATTRIBUTE.arguments, ast->ATTRIBUTE.arguments_num);
    Alc_String header = alc_string_create_from("ATTRIBUTE { name: \"");
    alc_string_append_cstr(&header, ast->ATTRIBUTE.name);
    alc_string_append_cstr(&header, "\", has_arguments: ");
    alc_string_append_cstr(&header, ast->ATTRIBUTE.has_arguments ? "true" : "false");
    alc_string_append_cstr(&header, " }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_ATTRIBUTE_LIST: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = safe_reserve(Alc_Vector(Alc_String), ast->ATTRIBUTE_LIST.attributes_num);
    array_to_strings(children_vs_v, ast->ATTRIBUTE_LIST.attributes,
                     ast->ATTRIBUTE_LIST.attributes_num);
  ...
  case ALC_AST_KIND_EXPLICIT_CALL_ARGUMENT: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->EXPLICIT_CALL_ARGUMENT.expression));
    Alc_String header = alc_string_create_from("EXPLICIT_CALL_ARGUMENT { name: \"");
    alc_string_append_cstr(&header, ast->EXPLICIT_CALL_ARGUMENT.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_LABEL: {
    ...
    alc_string_append_cstr(&header, ast->LABEL.name);
    alc_string_append_cstr(&header, "\" }");
    alc_vector_push(out_v, header);
    return out_v;
  }
  ...
  case ALC_AST_KIND_STMT_BLOCK: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = safe_reserve(Alc_Vector(Alc_String), ast->STMT_BLOCK.statements_num);
    array_to_strings(children_vs_v, ast->STMT_BLOCK.statements, ast->STMT_BLOCK.statements_num);
  ...
  case ALC_AST_KIND_STMT_RETURN: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    add_to_strings_opt(children_vs_v, ast->STMT_RETURN.expression);
  ...
  case ALC_AST_KIND_STMT_GOTO: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_GOTO.label));
  ...
  case ALC_AST_KIND_STMT_LABEL: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_LABEL.label));
  ...
  case ALC_AST_KIND_STMT_WHILE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 3);
    add_to_strings_opt(children_vs_v, ast->STMT_WHILE.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->STMT_WHILE.condition));
    alc_vector_push(children_vs_v, to_string(ast->STMT_WHILE.body));
  ...
  case ALC_AST_KIND_STMT_FOR: {
    ...
    add_to_strings_opt(children_vs_v, ast->STMT_FOR.attribute_list);
    add_to_strings_opt(children_vs_v, ast->STMT_FOR.init_statement);
    add_to_strings_opt(children_vs_v, ast->STMT_FOR.condition);
    add_to_strings_opt(children_vs_v, ast->STMT_FOR.expression);
    alc_vector_push(children_vs_v, to_string(ast->STMT_FOR.body));
  ...
  case ALC_AST_KIND_STMT_DO_WHILE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 3);
    add_to_strings_opt(children_vs_v, ast->STMT_DO_WHILE.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->STMT_DO_WHILE.condition));
    alc_vector_push(children_vs_v, to_string(ast->STMT_DO_WHILE.body));
  ...
  case ALC_AST_KIND_STMT_LOOP: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_LOOP.body));
  ...
  case ALC_AST_KIND_STMT_FOREACH: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 3);
    add_to_strings_opt(children_vs_v, ast->STMT_FOREACH.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->STMT_FOREACH.iterator));
    alc_vector_push(children_vs_v, to_string(ast->STMT_FOREACH.body));
    Alc_String header = alc_string_create_from("STMT_FOREACH { item_name: \"");
    alc_string_append_cstr(&header, ast->STMT_FOREACH.item_name);

    if (ast->STMT_FOREACH.i_name == nullptr)
      alc_string_append_cstr(&header, "\", i_name: - }");
    else {
      alc_string_append_cstr(&header, "\", i_name: \"");
      alc_string_append_cstr(&header, ast->STMT_FOREACH.i_name);
      alc_string_append_cstr(&header, "\" }");
    }
  ...
  case ALC_AST_KIND_STMT_EXPR: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_EXPR.expression));
  ...
  case ALC_AST_KIND_STMT_SWITCH: {
    Alc_Vector(Alc_Vector(Alc_String)) children_vs_v =
      alc_vector_reserve(Alc_Vector(Alc_String), ast->STMT_SWITCH.case_chains_num + 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_SWITCH.expression));
    array_to_strings(children_vs_v, ast->STMT_SWITCH.case_chains, ast->STMT_SWITCH.case_chains_num);
  ...
  case ALC_AST_KIND_STMT_DEFER: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_DEFER.body));
  ...
  case ALC_AST_KIND_STMT_IF: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 4);
    add_to_strings_opt(children_vs_v, ast->STMT_IF.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->STMT_IF.condition));
    alc_vector_push(children_vs_v, to_string(ast->STMT_IF.body));
    add_to_strings_opt(children_vs_v, ast->STMT_IF.else_statement);
  ...
  case ALC_AST_KIND_STMT_ELSE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->STMT_ELSE.body));
  ...
  case ALC_AST_KIND_TYPE_POINTER: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->TYPE_POINTER.type));
  ...
  case ALC_AST_KIND_TYPE_ARRAY: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 2);
    alc_vector_push(children_vs_v, to_string(ast->TYPE_ARRAY.type));
    add_to_strings_opt(children_vs_v, ast->TYPE_ARRAY.size_expression);
  ...
  case ALC_AST_KIND_TYPE_FUNCTION_POINTER: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 2);
    alc_vector_push(children_vs_v, to_string(ast->TYPE_FUNCTION_POINTER.argument_list));
    add_to_strings_opt(children_vs_v, ast->TYPE_FUNCTION_POINTER.return_type);
  ...
  case ALC_AST_KIND_TYPE_TYPE_OF: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->TYPE_TYPE_OF.expression));
  ...
  case ALC_AST_KIND_EXPR_OPERAND_IDENTIFIER: {
    ...
    alc_string_append_cstr(&header, ast->EXPR_OPERAND_IDENTIFIER.name);
    alc_string_append_cstr(&header, "\" }");
    alc_vector_push(out_v, header);
    return out_v;
  }
  ...
  case ALC_AST_KIND_EXPR_OPERAND_NUMBER: {
    Alc_String *out_v = alc_vector_reserve(Alc_String, 1);
    char buf[1024] = { 0 };
    if (ast->EXPR_OPERAND_NUMBER.typespec == nullptr)
      snprintf(buf, 1024, "EXPR_OPERAND_NUMBER { value: %" PRIu64 ", typespec: - }",
               ast->EXPR_OPERAND_NUMBER.value);
    else
      snprintf(buf, 1024, "EXPR_OPERAND_NUMBER { value: %" PRIu64 ", typespec: \"%s\" }",
               ast->EXPR_OPERAND_NUMBER.value, ast->EXPR_OPERAND_NUMBER.typespec);
  ...
  case ALC_AST_KIND_EXPR_OPERAND_NUMBER_FLOAT: {
    ...
    if (ast->EXPR_OPERAND_NUMBER.typespec == nullptr)
      snprintf(buf, 1024, "EXPR_OPERAND_NUMBER_FLOAT { value: %lf, typespec: - }",
               ast->EXPR_OPERAND_NUMBER_FLOAT.value);
    else
      snprintf(buf, 1024, "EXPR_OPERAND_NUMBER_FLOAT { value: %lf, typespec: \"%s\" }",
               ast->EXPR_OPERAND_NUMBER_FLOAT.value, ast->EXPR_OPERAND_NUMBER_FLOAT.typespec);
    alc_vector_push(out_v, alc_string_create_from(buf));
    return out_v;
  }
  ...
  case ALC_AST_KIND_EXPR_OPERAND_ARRAY_ELEMENT: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 2);
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_ARRAY_ELEMENT.array));
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_ARRAY_ELEMENT.index_expression));
  ...
  case ALC_AST_KIND_EXPR_OPERAND_CAST_TO: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 2);
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_CAST_TO.type));
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_CAST_TO.expression));
  ...
  case ALC_AST_KIND_EXPR_OPERAND_STRING: {
    Alc_String *out_v = alc_vector_reserve(Alc_String, 1);
    char buf[1024] = { 0 };
    if (ast->EXPR_OPERAND_STRING.typespec == nullptr)
      snprintf(buf, 1024, "EXPR_OPERAND_STRING { content: \"%s\", typespec: - }",
               ast->EXPR_OPERAND_STRING.content);
    else
      snprintf(buf, 1024, "EXPR_OPERAND_STRING { value: \"%s\", typespec: \"%s\" }",
               ast->EXPR_OPERAND_STRING.content, ast->EXPR_OPERAND_STRING.typespec);
    alc_vector_push(out_v, alc_string_create_from(buf));
    return out_v;
  }
  ...
  case ALC_AST_KIND_EXPR_OPERAND_SYMBOL: {
    Alc_String *out_v = alc_vector_reserve(Alc_String, 1);
    char buf[1024] = { 0 };
    if (ast->EXPR_OPERAND_SYMBOL.typespec == nullptr)
      snprintf(buf, 1024, "EXPR_OPERAND_SYMBOL { content: \"%s\", typespec: - }",
               ast->EXPR_OPERAND_SYMBOL.content);
    else
      snprintf(buf, 1024, "EXPR_OPERAND_SYMBOL { value: \"%s\", typespec: \"%s\" }",
               ast->EXPR_OPERAND_SYMBOL.content, ast->EXPR_OPERAND_SYMBOL.typespec);
    alc_vector_push(out_v, alc_string_create_from(buf));
    return out_v;
  }
  ...
  case ALC_AST_KIND_EXPR_OPERAND_ACCESS_FIELD_TUPLE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_ACCESS_FIELD_TUPLE.tuple));
    char buf[1024] = { 0 };
    snprintf(buf, 1024, "EXPR_OPERAND_ACCESS_MEMBER_TUPLE { index: %" PRIu64 " }",
             ast->EXPR_OPERAND_ACCESS_FIELD_TUPLE.index);
  ...
  case ALC_AST_KIND_EXPR_OPERAND_SIZE_OF: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_SIZE_OF.type));
  ...
  case ALC_AST_KIND_EXPR_OPERAND_ALIGN_OF: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_ALIGN_OF.expression));
  ...
  case ALC_AST_KIND_EXPR_OPERAND_OFFSET_OF: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 2);
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_OFFSET_OF.base_structure));
    alc_vector_push(children_vs_v, to_string(ast->EXPR_OPERAND_OFFSET_OF.field_expression));
  ...
  case ALC_AST_KIND_INITLIST: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = safe_reserve(Alc_Vector(Alc_String), ast->INITLIST.entries_num);
    array_to_strings(children_vs_v, ast->INITLIST.entries, ast->INITLIST.entries_num);
  ...
  case ALC_AST_KIND_INITLIST_ENTRY: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->INITLIST_ENTRY.expression));
  ...
  case ALC_AST_KIND_INITLIST_ENTRY_EXPLICIT: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    alc_vector_push(children_vs_v, to_string(ast->INITLIST_ENTRY_EXPLICIT.expression));
    Alc_String header = alc_string_create_from("INITLIST_ENTRY_EXPLICIT { field_name: \"");
    alc_string_append_cstr(&header, ast->INITLIST_ENTRY_EXPLICIT.field_name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_GENERIC_STRUCT: {
    Alc_Vector(Alc_Vector(Alc_String)) children_vs_v =
      alc_vector_reserve(Alc_Vector(Alc_String), ast->GENERIC_STRUCT.children_num + 2);
    add_to_strings_opt(children_vs_v, ast->GENERIC_STRUCT.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->GENERIC_STRUCT.generic_placeholder_type_list));
  ...
  case ALC_AST_KIND_GENERIC_FUNC: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 5);
    add_to_strings_opt(children_vs_v, ast->GENERIC_FUNC.attribute_list);
    alc_vector_push(children_vs_v, to_string(ast->GENERIC_FUNC.generic_placeholder_type_list));
    alc_vector_push(children_vs_v, to_string(ast->GENERIC_FUNC.argument_list));
    add_to_strings_opt(children_vs_v, ast->GENERIC_FUNC.return_type);
    alc_vector_push(children_vs_v, to_string(ast->GENERIC_FUNC.body));
    Alc_String header = alc_string_create_from("GENERIC_FUNC { name: \"");
    alc_string_append_cstr(&header, ast->GENERIC_FUNC.name);
    alc_string_append_cstr(&header, "\", kind: ");
    alc_string_append_cstr(&header, func_kind_to_string(ast->GENERIC_FUNC.kind));
    alc_string_append_cstr(&header, " }");
    return build_tree(header, children_vs_v);
  }

  case ALC_AST_KIND_GENERIC_PLACEHOLDER_TYPE_LIST: {
    Alc_Vector(Alc_Vector(Alc_String)) children_vs_v = safe_reserve(
      Alc_Vector(Alc_String), ast->GENERIC_PLACEHOLDER_TYPE_LIST.generic_placeholder_types_num);
    array_to_strings(children_vs_v, ast->GENERIC_PLACEHOLDER_TYPE_LIST.generic_placeholder_types,
                     ast->GENERIC_PLACEHOLDER_TYPE_LIST.generic_placeholder_types_num);
  ...
  case ALC_AST_KIND_GENERIC_PLACEHOLDER_TYPE: {
    Alc_Vector(Alc_Vector(Alc_String))
      children_vs_v = alc_vector_reserve(Alc_Vector(Alc_String), 1);
    add_to_strings_opt(children_vs_v, ast->GENERIC_PLACEHOLDER_TYPE.default_type);
    Alc_String header = alc_string_create_from("GENERIC_PLACEHOLDER_TYPE { name: \"");
    alc_string_append_cstr(&header, ast->GENERIC_PLACEHOLDER_TYPE.name);
    alc_string_append_cstr(&header, "\" }");
    return build_tree(header, children_vs_v);
  }
  ...
  case ALC_AST_KIND_GENERIC_TYPE_LIST: {
    Alc_Vector(Alc_Vector(Alc_String)) children_vs_v =
      safe_reserve(Alc_Vector(Alc_String), ast->GENERIC_TYPE_LIST.generic_types_num);
    array_to_strings(children_vs_v, ast->GENERIC_TYPE_LIST.generic_types,
                     ast->GENERIC_TYPE_LIST.generic_types_num);
