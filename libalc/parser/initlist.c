...
Alc_Ast *parse_initlist(Alc_Parser *p)
{
  ...
  Alc_Vector(Alc_Ast *) entries = alc_vector_create(Alc_Ast *);
  while (p->pos < p->tokens_num && p->tokens[p->pos].type != ALC_TOKEN_TYPE_RCBRACK) {
    Alc_Ast *entry = parse_entry(p);
    _VERIFY_AST(entry, { alc_vector_destroy(entries); });

    _VERIFY_POS(p, p->pos, { alc_vector_destroy(entries); });
  ...
  _VERIFY_POS(p, p->pos, { alc_vector_destroy(entries); });
  _VERIFY_TOKEN(p, p->pos, ALC_TOKEN_TYPE_RCBRACK, { alc_vector_destroy(entries); });
...
static Alc_Ast *parse_entry_explicit_array_elem(Alc_Parser *p)
{
  ...
  Alc_Vector(Alc_Ast *) index_expressions = alc_vector_create(Alc_Ast *);
  while (p->pos < p->tokens_num && p->tokens[p->pos].type == ALC_TOKEN_TYPE_LBRACK) {
    p->pos++;

    Alc_Ast *expr = parse_expr(p, false);
    _VERIFY_AST(expr, { alc_vector_destroy(index_expressions); });

    _VERIFY_POS(p, p->pos, { alc_vector_destroy(index_expressions); });
    _VERIFY_TOKEN(p, p->pos, ALC_TOKEN_TYPE_RBRACK, { alc_vector_destroy(index_expressions); });

    alc_vector_push(index_expressions, expr);

    p->pos++;
  }

  _VERIFY_POS(p, p->pos, { alc_vector_destroy(index_expressions); });
  _VERIFY_TOKEN(p, p->pos, ALC_TOKEN_TYPE_EQ, { alc_vector_destroy(index_expressions); });
